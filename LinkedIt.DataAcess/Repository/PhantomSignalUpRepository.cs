...
namespace LinkedIt.DataAcess.Repository
{
	public class PhantomSignalUpRepository : GenericRepository<PhantomSignalUp>, IPhantomSignalUpRepository
	{
		...
		{
			var up = await _db.PhantomSignalsUps.AnyAsync(u=>
		...
		public async Task<ICollection<string>> UserNamesUpPhantomSignalAsync(Guid phantomSignalId)
		{
			var userNames = await _db.PhantomSignalsUps
				.Where(u=>u.PhantomSignalId == phantomSignalId)
